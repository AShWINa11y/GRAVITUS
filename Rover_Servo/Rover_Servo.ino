...
#define SERVO_PIN 19 // <-- Added Servo Pin
...
int radarAngle = 0;
int radarDirection = 15; // Step by 15 degrees per tick
...
// ==========================================
// NON-BLOCKING RADAR LOGIC
// ==========================================
float getDistance() {
  digitalWrite(TRIG_PIN, LOW); delayMicroseconds(2);
  digitalWrite(TRIG_PIN, HIGH); delayMicroseconds(10);
  digitalWrite(TRIG_PIN, LOW);
  // 20000 microsecond timeout (limits max range to ~3.4 meters to save time)
...
void handle_radar() {
  // Check if 150 milliseconds have passed since the last step
  if (millis() - lastRadarTime >= 150) {
    lastRadarTime = millis(); // Reset stopwatch

    // 1. Take reading at the current angle
    currentDistance = getDistance();

    // Sound buzzer immediately if an obstacle is within 15cm at any angle
    if (currentDistance < 15.0) {
      digitalWrite(BUZZER_PIN, HIGH);
    } else if (!tippingHazard) { // Only turn off if IMU is also safe
      digitalWrite(BUZZER_PIN, LOW);
    }

    // 2. Calculate next angle
    radarAngle += radarDirection;

    // 3. Reverse direction if we hit the 0 or 180 degree limits
    if (radarAngle >= 180 || radarAngle <= 0) {
      radarDirection = -radarDirection;
    }

    // 4. Command servo to move to the new angle (It will travel during the next 150ms)
...
String get_dashboard_html() {
  ...
  html += "document.getElementById('ang').innerText = '@ ' + d.ang + ' deg';"; // Added Angle update
  ...
  html += "<div class='grid'>";
  // Radar card now shows distance AND angle
  html += "<div class='card'>Radar Scan<br><b id='dist'>-- cm</b> <span id='ang' style='font-size:12px; color:#aaa;'>@ -- deg</span></div>";
...
String get_sensor_json() {
  ...
  // Added the current radar angle ("ang") to the JSON output
  json += "{\"dist\":\"" + String(currentDistance, 1) + "\",\"ang\":\"" + String(radarAngle) + "\",\"volt\":\"" + String(volt, 2) + "\",";
...
void setup() {
  ...
  // Attach the servo and center it at 90 degrees on startup
